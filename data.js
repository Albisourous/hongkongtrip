...
const TRIP = {
  ...
  days: [
    ...
    },
    {
      ...
      checklist: [
        { t: "9:00am", task: "Check out · (opt) Sam's Tailor pickup, TST — last chance, it's off-route after today · MTR → West Kowloon HSR terminus — security + border ≥45 min before departure; gates open ~15 min, close 5 min" },
    ...
    },
    {
      ...
      checklist: [
        ...
        { t: "3:30pm", task: "(opt) Hanfu dress-up — Fanhua Hanfu studio in Yongqingfang: men's 战国袍 robes / kung-fu fits, rental+makeup ~¥150+, photo packages ~¥370+ — eats the Shamian slot" },
    ...
    {
      date: "Oct 2", day: "Fri", base: "Macau → HK (Lantau)", stay: "SkyCity Marriott (airport)", legs: ["hk2"],
      notes: "Cross on the HZMB bridge, not the ferry — it lands right by the airport so the whole day stays on Lantau. Default: 24h Gold Bus shuttle Macau Port → HK Port (~HK$65) + B4 to HKIA (~HK$9), no booking needed. Upgrade option: Macau HK Airport Direct coach Macau Port → HKIA SkyPier (~45 min, ~HK$280, bags checked in Macau, skips HK immigration) — but it's an airside service needing an eligible onward flight, confirm the Oct 3 9:25am flight qualifies before relying on it. Ngong Ping 360 runs ~10:00–18:00 weekdays (book timed tickets); Po Lin Monastery does a veg set lunch. Sam's Tailor pickup has to happen before leaving HK on Sep 28 — not today. Last real meal is Tung Chung — airport area is slim pickings.",
      checklist: [
        { t: "8:00am", task: "Margaret's Café e Nata egg tarts to-go (opens ~8:30) · checkout Casa Real" },
        { t: "9:00am", task: "Taxi/bus → HZMB Macau Port (港珠澳大桥澳门口岸)" },
        { t: "9:30am", task: "Cross border → Gold Bus 金巴 over HZMB → Hong Kong Port (~45 min, 24h, ~HK$65) — (opt) Airport Direct coach to SkyPier if flight-eligible", must: true },
        { t: "11:00am", task: "B4 bus → HKIA (~10 min, ~HK$9) → SkyCity Marriott — drop bags (check-in 3pm)" },
        { t: "12:00pm", task: "Taxi/bus S1 → Tung Chung · Ngong Ping 360 cable car up (~25 min) — (opt) Crystal Cabin glass-bottom upgrade" },
        { t: "12:45pm", task: "Tian Tan Buddha — 268 steps + Po Lin Monastery veg lunch", must: true },
        { t: "2:15pm", task: "Wisdom Path loop (~30–45 min) — still in Ngong Ping" },
        { t: "3:00pm", task: "(opt) Bus 21 → Tai O fishing village — stilt houses + boat ride (~20 min each way) — skip = linger at Ngong Ping" },
  ...
  resources: [
    ...
    { cat: "Bus Macau→HK", detail: "HZMB crossing to the airport side: Gold Bus 金巴 Macau Port → HK Port, 24h ~45 min (~HK$65) + B4 → HKIA (~HK$9) — landside, no booking. Upgrade: Macau HK Airport Direct coach → SkyPier airside, ~45 min ~HK$280, bags checked in Macau — needs an eligible onward flight.", cost: "~$10–36", url: "https://www.macauhkairportbus.com", linkLabel: "macauhkairportbus.com" },
  ...
  places: [
    ...
    { name: "Yongqingfang + Bruce Lee home", leg: "gz", cat: "see", day: "Sep 29", lat: 23.1159, lng: 113.2396, note: "(opt) Fanhua Hanfu studio here — men's hanfu rental + photo shoots ~¥150–370+" },
    ...
    { name: "HZMB Macau Port", leg: "mo", cat: "move", day: "Oct 2", lat: 22.2129, lng: 113.5783, note: "Gold Bus + Airport Direct coaches to HK depart here" },
    ...
    // Hong Kong airport / Lantau — hk2
    { name: "HZMB Hong Kong Port", leg: "hk2", cat: "move", day: "Oct 2", lat: 22.3172, lng: 113.9504, note: "Gold Bus arrives from Macau → B4 to HKIA (~10 min)" },
    ...
    { name: "Tai O fishing village", leg: "hk2", cat: "see", day: "Oct 2 (opt)", lat: 22.2544, lng: 113.8624, note: "stilt houses · boat ride · bus 21 from Ngong Ping ~20 min" },
  ...
  costs: [
    ...
    { id: "bus-mo-hk", leg: "hk2", cat: "Transport", label: "Macau → HKIA via HZMB (Gold Bus + B4)", total: null, perPerson: 10, frontedBy: null, status: null, note: "No pre-booking needed · Airport Direct coach ~HK$280 if flight-eligible" },
    { id: "taxi-skycity", leg: "hk2", cat: "Transport", label: "SkyCity ↔ Tung Chung/Ngong Ping shuttles", total: null, perPerson: null, frontedBy: null, status: null, note: "S1 bus ~HK$4 or ~HK$60 cab split · hotel shuttle runs to the airport" },
